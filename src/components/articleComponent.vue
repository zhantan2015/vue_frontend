<template>
    <article>
        ...
        <footer>
            ...
            <span class="continue_reading" @click="edit">
                编辑
            </span>
            <span class="continue_reading" @click="del">
                删除
            </span>
            <div class="article_info" :class="{ dark: dark }">
                ...
                <div class="tags_div" v-if="article.tags && article.tags.length > 0">
                    ...
                    <span v-for="tag in article.tags" :key="tag" v-text="tag"></span>
                </div>
            </div>
        </footer>
    </article>
...
import { mainState, articleState } from '@/store'
...
const { dark } = storeToRefs(mainStateObj)
function continueReading() {
    router.push('/article/' + article.aid)
}
function edit() {
    router.push('/edit-article/' + article.aid)
}
function del() {
    requests.delete('/article/' + article.aid)
        .then(res => {
            mainStateObj.activeNotification(res)
            if (res.type == 'success') articleStateObj.delArticleByAid(article.aid)
        })
}
